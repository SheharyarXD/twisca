...
const OrderSummary=({ orders, totalPrice, discount, estimatedDelivery })=>{
  ...
  const {cart,
    ...
    return(
        <div className="flex flex-col z-[40] w-full min-h-fit border-gray-300 border px-[2vw] rounded-lg">
...
/>
            ...
            {cartToggle==="cartP"&&(
                <div> <button className="bg-[#8B024B] text-white px-4 py-2 rounded-[1vw] md:rounded-[0.5vw] mt-[0.5vh] w-full" onClick={()=>{
                    // userid===0?navigate("/login"):
                    setcartToggle("billing")
                }}>Continue</button></div>
            )}
